...
import { InsertVisaCategory, InsertVisaCategoryTranslate, visaCategories, visaCategoryTranslate } from "../entities";
...
export class VisaCategoryRepository {
  ...
  async remove(id: number): Promise<any> {
    await this._drizzle.db().delete(visaCategories).where(eq(visaCategories.id, id));
  }
}