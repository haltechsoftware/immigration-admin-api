...
import { departureRegistration, passportInformation, personalInformation } from '../entities';
...
export class DepartureRepository {
  ...
  async create({ input }: DepartureRegistrationCommand): Promise<string> {
    try {
      const { passport_info, personal_info } = input;
  
      return await this.drizzle.db().transaction(async (tx) => {
        // Insert Personal Information
        const [personalInfoRes] = await tx
          .insert(personalInformation)
          .values({
            ...personal_info,
            race: "",
            family_name: personal_info.family_name,
            date_of_birth: format(personal_info.date_of_birth, DateTimeFormat.date),
            phone_number: personal_info.phone_number,
            place_of_birth: personal_info.place_of_birth,
          });
  
        // Insert Passport Information
        const [passportInfoRes] = await tx
          .insert(passportInformation)
          .values({
        ...
        await tx.insert(departureRegistration).values({
