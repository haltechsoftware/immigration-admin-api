  ...
  implements ICommandHandler<ArrivalRegistrationCommand, string>
{
  ...
  async execute({ input }: ArrivalRegistrationCommand): Promise<string> {
    ...
    if (!isValidCountryId) {
        throw new BadRequestException('Invalid country_id: must be a valid integer.');
    }
    ...
    if (!country) {
        throw new NotFoundException({ message: `${countryId} ລະຫັດບໍ່ມີໃນລະບົບ` });
    }

    if (input.visa?.date_of_issue) validateDate(input.visa.date_of_issue, 'visa.date_of_issue');
    if (input.personal_info?.date_of_birth) validateDate(input.personal_info.date_of_birth, 'date_of_birth');
    if (input.passport_info?.expiry_date) validateDate(input.passport_info.expiry_date, 'expiry_date');
    if (input.passport_info?.date_of_issue) validateDate(input.passport_info.date_of_issue, 'passport_info.date_of_issue');

    const passports = await this.repository.getPassportAll(input.passport_info.no);
