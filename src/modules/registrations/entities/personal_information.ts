...
import { nationality } from 'src/modules/nationality/entities';

export const genderPersonalInfo = mysqlEnum('gender', ['female', 'male']);

export const personalInformation = mysqlTable(
  ...
  (table) => ({
    nationalityByReference: foreignKey({
      columns: [table.nationality_id],
...
export const personalInformationRelations = relations(
  ...
  ({ one }) => ({
    nationality: one(nationality, {
      fields: [personalInformation.nationality_id],
